import './start.css'
const Start=()=>
{
    return(<div>
        <div className="start"></div>
            <p className='start_para'>
             <h1 className="heading  animate__animated  animate__fadeInLeft"><i><span className="left-quote">“</span>Fitness is a Lifestyle not a Destination<span className="right-quote">”</span></i> </h1>
                <p className='mx-auto pt-0'> Goals are meant to be achieved, records are meant to be broken.
                Be it any goal muscle building,fat loss,weight loss or just maintenence we have the right guidance on how to 
                achieve and shape your body.</p>
               <button type="button" class="btn btn-dark start_btn">GET STARTED</button>
            </p>
        
</div>
    )
}
export default Start;